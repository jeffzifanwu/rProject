...
df.simData = df.simData %>%
  mutate(treatment_group = case_when(
      site_id <= 1/4*nSite ~ 2 # treated on period 2
    , site_id <= 2/4*nSite ~ 3 # treated on period 3
    , site_id <= 3/4*nSite ~ 4 # treated on period 4
    , .default = 0 # external sites - never treated
  )) %>%
  mutate(in_treatment_group = if_else(
      treatment_group %in% c(2, 3, 4)
    , TRUE
    , FALSE
  ))
...
df.simData = df.simData %>%
  mutate(post_policy = case_when(
      treatment_group == 2 & period_id >= 2 ~ TRUE
    , treatment_group == 3 & period_id >= 3 ~ TRUE
    , treatment_group == 4 & period_id >= 4 ~ TRUE
    , .default = FALSE
  )) 
