...
Assuming that we do not have the logit probability function @eq-logitProb, we would have to simulate the probability of choosing alternative 1. We can use the following steps:

In the previous section, we estimated the coefficients using maximum likelihood estimation (MLE). However, in practice, we may not have access to a closed-form function for the probability of choosing alternative 1 (e.g., the logit probability @eq-logitProb). In such cases, we simulate the probability of choosing alternative 1 conditional on some paramaters and predictors. The simulated probability is given by:
...
nlls <- function(beta, x, y, R) {
  
  # deterministic utility difference
  mat.v = x %*% beta
  
  # mpty matrix to store simulated choices
  mat.ysim = matrix(0, nrow = nrow(x), ncol = R)
  
  # simulate choices
  for (r in 1:R) {
    # simulate error terms for all individuals in the r-th simulation
    mat.eps = rgumbel(nrow(x), loc = 0, scale = 1)
    
    # calculate total utility
    mat.vTotal = mat.v + mat.eps
...
# var-cov matrix
mat.varCov = solve(opt_sim$hessian)
