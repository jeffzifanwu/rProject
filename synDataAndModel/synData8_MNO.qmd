...
-   @sec-stdSetup: Standard MNL model
...
# Standard MNL model {#sec-stdSetup}
...
# parameters
alphaTrue = c(
  1,   # alpha1, alternative 0
  2,   # alpha1, alternative 1
  3,   # alpha1, alternative 2
...
df.sim = df.sim %>%
  mutate(x2 = case_when(
    alter_id == 0 ~ rnorm(n(), 50, 10),
    alter_id == 1 ~ rnorm(n(), 45, 10),
    alter_id == 2 ~ rnorm(n(), 40, 10)
  ))
...
df.sim = df.sim %>%
  mutate(eps = rgumbel(N*J, loc = 0, scale = 1))
...
One of the three intercepts, $\alpha_i^1$, $i = 0,1,2$, is not identifiable. If they all change freely, the probability is not uniquely determined by the parameter values. For example, $\boldsymbol{\alpha}^{1} = (0,1,2)$ and $\boldsymbol{\alpha}^{1'} = (1,2,3)$ yield the same choice probability. We set:
...
\begin{align}
  \beta^{1}_{0} 
    ...
    \\
  \beta^{1}_{1} 
    & = \alpha^{1}_{1} -  \alpha^{1}_{0}
    \\
  \beta^{1}_{2}
    & = \alpha^{1}_{2} -  \alpha^{1}_{0}
    \\
  \beta^{2}_i 
    & = \alpha^{2}_i \ , \ \forall i
    \\
...
The parameters we will estiamte are: $\beta_{1}^{1}$, $\beta_{2}^{1}$, $\beta_{i}^{2}$, $\forall i$, $\beta_{i}^{3}$, $\forall i$.
...
nll <- function(beta, mat.x, mat.y) {
  ...
  # Developer variables
  # beta = c(0.66, 1.09, 0.12, 0.24)
  # beta = c(1, 2, 0.2, 0.6)
  
  # debug: check mat.x and beta match
  if (ncol(mat.x) != length(beta) + 5) {
    stop("Error: 'mat.x' must have one more column than the length of 'beta'.")
  }
  ...
  mat.p_0 = exp(mat.v_0) / as.matrix(rowSums(exp(mat.v)))
  mat.p_1 = exp(mat.v_1) / as.matrix(rowSums(exp(mat.v)))
  mat.p_2 = exp(mat.v_2) / as.matrix(rowSums(exp(mat.v)))
...
betaTrue = c(
  ...
  0.2, # alpha2, alternative 0,1,2
  0.6  # alpha3, alternative 2,1,2
) # we know this is true

nllCurr = nll(betaTrue, mat.x, mat.y) # current total negative log-likelihood
...
# start values
startValues = rep(0,ncol(mat.x)-5)
...
The parameters are recovered.

# Scale parameter is not one {#sec-scaleParam}

In reality, the scale parameter of $\epsilon$ might not be one i.e., $\sigma \neq 1$.

We can only identify:

$$
\begin{align}
  \frac{\beta^{1}_{i} }{\sigma}
    & = \frac{\alpha^{1}_{i}}{\sigma} -  \frac{\alpha^{1}_{0}}{\sigma} \ , \ \forall i =1,2
    \\
  \frac{\beta^{2}_i}{\sigma} 
    & = \frac{\alpha^{2}_i}{\sigma} \ , \ \forall i=0,1,2
    \\
  \frac{\beta^{3}_i}{\sigma} 
    & = \frac{\alpha^{3}_i}{\sigma} \ , \ \forall i=0,1,2
    \\
\end{align}
$$

To demonstrate this, we simulate a dataset with

$$
\epsilon_{n,i} \overset{i.i.d}{\sim} EV1 (0,2)
$$

We re-use most parts of the R codes in @sec-stdSetup. The only change is as follows:

`df.sim = df.sim %>% mutate(eps = rgumbel(N*J, loc = 0, scale = 2))`
...
# unobserved utilities (error terms)
df.sim = df.sim %>%
  mutate(eps = rgumbel(N*J, loc = 0, scale = 2))
...
As anticipated, we have recovered:
...
In this numerical experiment, we know $\simga = 2$ because we simulated the dataset. In practice, however, $\sigma$ is unknown. This means that the levels (absolute sizes) of the parameters are not identifiable in general.

# Predictor is not alternative specific

Similar to the intercepts, when one of the predictors is not alternative specific e.g., $x^1_{0} = x^1_{1} = x^1_{2}$, we cannot identify:

$$
\frac{\beta^{2}_i}{\sigma} \ , \ \forall i=0,1,2
$$

Only the differences are identifiable. We define:

$$
\frac{\theta^{2}_i}{\sigma} 
= \frac{\beta^{2}_i}{\sigma} - \frac{\beta^{2}_0}{\sigma}
= \frac{\alpha^{2}_i}{\sigma} - \frac{\alpha^{2}_0}{\sigma} \ , \ \forall i=1,2
$$

We re-use most parts of the R codes from @sec-scaleParam.

# Others

**Utility differences**

Agent specific predictor
