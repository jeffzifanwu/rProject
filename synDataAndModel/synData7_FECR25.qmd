...
The point estiamtes are the same, but the standard errors differ. As of this post is created (07 June 2025),  `clubSandwich` does not support `fixest`-style models (see 
[readme](https://github.com/jepusto/clubSandwich) and 
[issues](https://github.com/jepusto/clubSandwich/issues/61))
)
...
The estiamtion results are the same as `lm()`. This syntax does not harness the superior algorithm in `fixest` that enhances its performance in estimating high-dimensional fixed effects. However, many practical features of `fixest` are still available. Most notably, for dynamic TWFE models, we can use the `i()` function to specify interaction terms between time-to-treatment and treatment group variables. We will demonstrate this in the next section.
...
Note that the `Time_to_Treatment` variable is `NA` for control units. We set `Time_to_Treatment` to -1 for control units, because we will set `Time_to_Treatment = -1` to be reference period later, i.e., $\theta_{g,-1} = 0$, $\forall g$.
